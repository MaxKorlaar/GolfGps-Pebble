...
var courseWindow = new UI.Window({  status: false,  backgroundColor:'white',  color:'black',  action: {    up: 'images/up_arrow.png',    down: 'images/down_arrow.png',    select: 'images/play.png'  }});
...
courseWindow.on('click',"up", function(e) {
	courseResultNum--;
...
courseWindow.on('click',"down", function(e) {
		courseResultNum++;
...
function updateTeeMenu(course_id){
	var items = [];
	for(var i=0;i<course.tees.length;i++){
		items[i] = {title:course.tees.tee_type+" "+course.tees.tee_color,subtitle:	"length "+course.tees.length+"m par "+course.tees.par};
	}
	teeMenu.items(0, items);
}
...
homeWindow.on('click', function(e) {
  courseWindow.show();
});
homeWindow.show();
...
distanceWindow.on('click',"up", function(e) {
	nextItem();
});
distanceWindow.on('longClick',"up", function(e) {
	hazardNum = holes[hole].items.length-1;//Go to the green
	nextItem();
});


distanceWindow.on('click',"down", function(e) {
	previousItem();
});
distanceWindow.on('longClick',"down", function(e) {
	hazardNum=0;
	previousItem();
});
...
distanceWindow.on('click',"select", function(e) {
	startTracking();
});
...
clubsMenu.on('click','back', function(e) {
	trackingClub = -2;
...
trackingWindow.on('click',"up", function(e) {
	var distanceTracked = distance(trackingFrom);
...
trackingWindow.on('click',"down", function(e) {
	trackingClub = -2;
	trackingWindow.hide();
	distanceExtraText.text("");
});
...
function getCourses(){
	...
	ajax({ url: 'http://pieteroskam.nl/pebbleGolf/apiv2.php?radius='+options.searchRadius+'&lat='+position.coords.latitude+'&lon='+position.coords.longitude+'&email='+options.email, type: 'json' },
